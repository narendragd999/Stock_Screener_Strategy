...
def main():
   ...
   #     st.markdown("### Filter and Sort Stocks")
   #     col1, col2 = st.columns([3, 1])
   #     with col1:
   #         search_query = st.text_input("Search by Symbol or Strategy", "", key="stock_search")
   #     with col2:
   #         sort_column = st.selectbox(
   #             "Sort By",
   #             options=["symbol", "strategy", "initial_price", "alert_price", "target_price", "current_price"],
   #             key="sort_column"
   #         )
   ...
   #     current_prices = {}
   #     for _, row in df.iterrows():
   #         current_price, _, _ = data_fetcher.get_stock_data(row['symbol'])
   #         current_prices[row['id']] = current_price if current_price else 0
   #     df['current_price'] = df['id'].map(current_prices)
   ...
   #     if search_query:
   #         df = df[
   #             df['symbol'].str.contains(search_query.upper(), case=False, na=False) |
   #             df['strategy'].str.contains(search_query, case=False, na=False)
   #         ]
   ...
   #     # Display stock table
   #     st.dataframe(
   #         df[['S.No', 'symbol', 'strategy', 'initial_price', 'alert_price', 'target_price', 'current_price', 'alert_triggered', 'created_at']],
   #         use_container_width=True,
   #         column_config={
   #             'S.No': st.column_config.NumberColumn(width="small"),
   #             'symbol': st.column_config.TextColumn("Symbol"),
   #             'strategy': st.column_config.TextColumn("Strategy"),
   #             'initial_price': st.column_config.NumberColumn("Initial Price (₹)", format="%.2f"),
   ...
   #             'alert_triggered': st.column_config.CheckboxColumn("Alert Triggered"),
   ...
   #     )
       ...
       #                 db_manager.toggle_stock_status(row['id'], row['enabled'])
       #                 st.rerun()
       #         with col4:
       ...
       #                     db_manager.update_stock_prices(row['id'], new_alert_price, new_target_price)
       ...
       #             elif row['strategy'] == '52WLowHigh' and row['alert_price'] > 0:
       #                 distance_from_52w_low = ((current_price - row['alert_price']) / row['alert_price'] * 100)
       #                 st.write(f"Distance from 52W Low (%): {distance_from_52w_low:.2f}")
       #         else:
